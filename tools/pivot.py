...
def get_pivot(path):
    ...
    occ = read_dwca(path, ['decimalLatitude', 'decimalLongitude', 'species']).dropna()
...
def store_per_latitude(dataframe, folder, from_degrees, stepsize):
    ...
    while from_degrees < 90:
        selection = dataframe.query(
            'decimalLatitude > ' + str(from_degrees) + ' & decimalLatitude < ' + str(from_degrees + stepsize))
        if len(selection):
...
def get_cells(path):
    occ = read_dwca(path, ['decimalLatitude', 'decimalLongitude']).dropna()
    occ['decimalLatitude'] = np.floor(occ['decimalLatitude'] * 120) / 120 + (1 / 240)
    occ['decimalLongitude'] = np.floor(occ['decimalLongitude'] * 120) / 120 + (1 / 240)
    occ.drop_duplicates(inplace=True)
    return occ
...
if __name__ == '__main__':
    ...
    # file = '/home/wouter/Projects/Naturalis/datasets/GBIF_Bombus_1990-2021_acc1000.zip'
